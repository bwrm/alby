...
class Command(BaseCommand):
    help = _("Initialize the workdir to run the demo of alby.")
    ...
    def handle(self, verbosity, *args, **options):
        ...
        if os.path.isfile(initialize_file):
            self.stdout.write("Initializing project alby")
            call_command('makemigrations', 'alby')
            call_command('migrate')
            ...
            #call_command('loaddata', 'products-media') #load dummy products
            #call_command('import_products')
            #self.create_polymorphic_subcategories()
            #call_command('initialize_inventories')
        else:
            self.stdout.write("Project alby already initialized")
            call_command('migrate')
